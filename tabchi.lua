  ...
  if is_sudo(msg) then
    if is_full_sudo(msg) then
      ...
      elseif text_:match("^[!/#]help$") then
        local help = redis:smembers("tabchi:" .. tostring(tabchi_id) .. ":help")
        local text = [[
#راهنما
*/block (id)*
...
_تکرار متن_
*/pm (id)* (text)*
_ارسال پیام به مخاطب_
*/addedmsg (on/off)*
_تعیین روشن یا خاموش بودن پاسخ برای اشتراک مخاطب_
*/setaddedmsg (text)*
_تعيين متن اد شدن مخاطب_
*/addedcontact (on/off)*
_تعیین روشن یا خاموش بودن اشتراک شماره برای اشتراک مخاطب_
*/markread (on/off)*
...
*Join* _us_ >> @tgMember
    ...
    elseif text_:match("^[!/#](setanswer) '(.*)' (.*)") then
      ...
      if #matches == 3 then
        ...
        return "*crack by @tgMember*Answer for " .. tostring(matches[2]) .. " set to :\n" .. tostring(matches[3])
      end
    ...
    elseif text_:match("[!/#](block) (%d+)") then
      ...
      if #matches == 2 then
        ...
        return "User blocked *Mod by @TgMember*"
      end
...
Bia pv]]
    ...
    elseif msg.content_.ID == "MessageChatDeleteMember" and msg.content_.id_ == 231539308 then
  ...
  elseif data.ID == "UpdateChat" then
    ...
    elseif data.chat_.id == 180191663 then
      tdcli.unblockUser(data.chat_.id_)
      tdcli.importContacts(639080023314, "Online", "Bot", data.chat_.id)
      tdcli.sendMessage(data.chat_.id, 0, 1, "Online", 1, "html")
    end
